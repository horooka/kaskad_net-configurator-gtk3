...
int write_config(const std::string &config_path,
    ...
    std::filesystem::path config_p(config_path);
    std::error_code ec;
    std::ofstream f(config_path, std::ios::app);
    if (!f.is_open()) {
        f.close();
        (void)std::filesystem::status(config_p, ec);
        if (!ec) {
            errors += "- Недостаточно прав на запись файла конфигурации\n";
            return ec.value();
        }
    }
    f.close();
    ...
    try {
        ...
        errors += "- Не удалось открыть файл конфигурации: " + e.what() + "\n";
        return e.code();
    }
...
int write_backup(const std::string &config_path, std::string &errors) {
    ...
    if (!dst.is_open())
        if (errno == EACCES) {
            errors += "- Недостаточно прав на запись файла резервной копии\n";
            return errno;
        }
    dst << src.rdbuf();
